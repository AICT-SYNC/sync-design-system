...
export const useDatePicker = ({
  ...
  const $month = Number(date[1]) || today.getMonth() + 1;
  const $day = Number(date[2]) || today.getDate();
  ...
  const onChangeSelectDate = useCallback(
    (day: number) => {
      ...
      setFold(true); // 날짜 선택 후 캘린더 닫기
    },
    [setSelectDate, calendarDate.year, calendarDate.month, onChange, setFold]
  );
